/**
 * Express alapú backend szerver a FókuszMester alkalmazáshoz.
 * Tartalmazza az összes szükséges API végpontot:
 * - felhasználói regisztráció, bejelentkezés, jelszókezelés
 * - tananyagok és kvízek lekérdezése
 * - súgó (GYIK) cikkek keresése
 * - admin funkciók (pl. teszt adatok törlése)
 */
...
const fs = require('fs/promises');     // aszinkron fájlműveletekhez
const validator = require('validator');
// Rate limiter modul (express-rate-limit) importálása biztonságos IP kulcsgenerálással
...
// Rate limiter beállítása (15 perc alatt max 10 próbálkozás egy IP+email kombinációra)
const authLimiter = rateLimit({
    ...
    keyGenerator: (req, res) => {
        // A kulcsot a biztonságosan kezelt IP címből ÉS a kérésben szereplő e-mail címből (ha van) rakjuk össze.
...
// A route modulok betöltése **a pool és authenticateToken definíciója után**:
const parentsRouter  = require('./routes/parents')(pool, authenticateToken);
const teachersRouter = require('./routes/teachers')(pool, authenticateToken);
const classesRouter  = require('./routes/classes')(pool, authenticateToken);
const reportsRouter  = require('./routes/reports')(pool, authenticateToken);

// Route modulok regisztrálása az alkalmazásban:
...
app.post('/api/register', authLimiter, async (req, res) => {
    ...
    try {
        ...
        const verificationExpires = new Date(Date.now() + 3600000); // 1 óra lejárati idő
        const newReferralCode = `FKSZ-${crypto.randomBytes(6).toString('hex').toUpperCase()}`;
        const newUserQuery = `
            INSERT INTO Users (username, email, password_hash, role, referral_code, email_verification_token, email_verification_expires) 
        ...
        const userMailOptions = {
            ...
            html: `<p>Kattints a linkre a regisztrációdhoz: <a href="${verificationUrl}">Megerősítés</a></p>`
        };
...
app.post('/api/classes/create', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const newClassResult = await pool.query(
            'INSERT INTO Classes (class_name, class_code, teacher_id, max_students) VALUES ($1, $2, $3, $4) RETURNING *;',
...
app.get('/api/teacher/classes', authenticateToken, async (req, res) => {
    ...
    try {
        const classesQuery = `
            SELECT c.*, COUNT(cm.user_id)::int AS student_count
            FROM Classes c
...
app.get('/api/curriculums', async (req, res) => {
    ...
    try {
        ...
        if (subject || grade || q) {
            // Ha volt szűrés, a találatokat közvetlenül visszaadjuk
            return res.status(200).json({ success: true, data: result.rows });
        }
        // Ha nem volt szűrés, csoportosítva adunk vissza bizonyos kategóriákat
