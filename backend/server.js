...
app.get('/api/learning-paths/:quizSlug', authenticateTokenOptional, async (req, res) => {
    try {
        ...
        if (!quizSlug) {
            return res.status(400).json({ success: false, message: 'Hiányzó kvíz azonosító.' });
        }

        const curriculumResult = await pool.query('SELECT id, title FROM curriculums WHERE slug = $1', [quizSlug]);
        if (curriculumResult.rows.length === 0) {
            return res.status(404).json({ success: false, message: 'A megadott kvíz/tananyag nem található.' });
        }
        const curriculumId = curriculumResult.rows[0].id;
        const curriculumTitle = curriculumResult.rows[0].title;

        const lessonsQuery = `
            SELECT
                qlp.difficulty_level,
                l.id AS lesson_id,
                l.title AS lesson_title,
                l.description AS lesson_description,
                l.content_slug AS lesson_content_slug,
                qlp.order_index
            FROM quiz_learning_paths qlp
            JOIN lessons l ON qlp.lesson_id = l.id
            WHERE qlp.curriculum_id = $1
            ORDER BY qlp.difficulty_level, qlp.order_index;
        `;
        ...
        rawLessons.forEach(lesson => {
            const formattedLesson = {
                ...
                content_slug: lesson.lesson_content_slug,
            };
            if (learningPaths[lesson.difficulty_level]) {
                learningPaths[lesson.difficulty_level].push(formattedLesson);
            }
        });
        ...
        res.status(200).json({
            success: true,
            quizSlug: quizSlug,
            quizTitle: curriculumTitle,
            learningPaths: learningPaths,
        });

    } catch (err) {
        logger.error(`❌ Hiba a(z) /api/learning-paths/${req.params.quizSlug} lekérdezésekor:`, { message: err.message, stack: err.stack });
        res.status(500).json({ success: false, message: 'Szerverhiba történt a tanulási útvonalak lekérdezésekor.' });
    }
});
